// backend/middleware/authorizePermiso.js
...
async function getRolIdFromReq(req) {
  // Ideal: auth ya pone req.user.rol_id
  if (req.user?.rol_id) return Number(req.user.rol_id);

  // Fallback: buscar por usuario_id
  if (!req.user?.id) return null;
  const [[u]] = await exec(`SELECT rol_id FROM usuarios WHERE id = ? LIMIT 1`, [
    Number(req.user.id),
  ]);
  ...
  return async (req, res, next) => {
    try {
      ...
      return res.status(403).json({
        ok: false,
        message: "No autorizado (permiso requerido).",
        permiso: clave,
      });
    } catch (e) {
      next(e);
...
export function requirePermisos(...claves) {
  return async (req, res, next) => {
    try {
      ...
      return res.status(403).json({
        ok: false,
        message: "No autorizado (faltan permisos).",
        faltan,
      });
    } catch (e) {
      next(e);
