...
function normalizeEstado(v) {
  ...
  if (["EN_PREPARACION", "EN PREPARACION", "PREPARANDO", "COCINANDO", "IN_PROGRESS"].includes(s))
...
async function fetchOrdenes(params = {}) {
  const candidates = [
    { url: "/api/ordenes", params },
    { url: "/api/ordenes/list", params },
    { url: "/api/reportes/ordenes", params },
  ];
...
async function fetchOrdenDetalle(id) {
  const candidates = [`/api/ordenes/${id}`, `/api/ordenes/detalle/${id}`];
  let lastErr = null;
...
export default function Ordenes() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const list = await fetchOrdenes({ desde, hasta, estado: estado || undefined, tipo: tipo || undefined, q: q || undefined });

      const normalized = list.map((o) => {
        const createdAt = o.created_at || o.fecha || o.fecha_creacion || o.createdAt || o.fecha_hora || null;
        const codigo = o.codigo || o.correlativo || o.numero || `#${o.id}`;
        const est = normalizeEstado(o.estado);
        const items = safeArray(o.items) || safeArray(o.detalle) || safeArray(o.detalles) || safeArray(o.productos);
      ...
      setMsg({
        type: "danger",
        text: e?.response?.data?.message || "No se pudieron cargar órdenes (revisa GET /api/ordenes en backend).",
      });
    } finally {
      setLoading(false);
    }
  ...
  const filtradas = useMemo(() => {
    ...
    if (text) {
      list = list.filter((o) => {
        const base = `${o.codigo} ${o.tipo} ${o.mesa ?? ""} ${o.estado} ${o.cliente_nombre ?? ""} ${o.cliente ?? ""}`.toLowerCase();
  ...
  const openDetalle = async (orden) => {
    ...
    setOrdenSel(null);
    try {
      const hasItems = safeArray(orden.items).length > 0;
      if (hasItems) {
        setOrdenSel(orden);
      } else {
        const det = await fetchOrdenDetalle(orden.id);
        const createdAt = det?.created_at || det?.fecha || det?.createdAt || orden.createdAt;
        const items = safeArray(det?.items) || safeArray(det?.detalle) || safeArray(det?.detalles) || safeArray(det?.productos);
        setOrdenSel({ ...orden, ...det, createdAt, estado: normalizeEstado(det?.estado ?? orden.estado), items });
      }
    } catch (e) {
      setMsg({ type: "danger", text: e?.response?.data?.message || "No se pudo cargar el detalle." });
  ...
  const cambiarEstado = async (orden, estadoNuevo) => {
    ...
    try {
      ...
      setMsg({ type: "success", text: `✅ ${orden.codigo} → ${estadoNuevo.replace("_", " ")}` });
      await load();
    } catch (e) {
  ...
  const acciones = (o) => {
    ...
    return (
      <div className="d-flex gap-2 justify-content-end flex-wrap">
        <Button size="sm" variant="outline-dark" className="d-inline-flex align-items-center gap-2" onClick={() => openDetalle(o)}>
          <FaEye /> Ver
        </Button>

        {est === "NUEVA" ? (
          <Button size="sm" variant="warning" disabled={disabled} onClick={() => cambiarEstado(o, "EN_PREPARACION")}
        ...
        {est === "EN_PREPARACION" ? (
          <Button size="sm" variant="success" disabled={disabled} onClick={() => cambiarEstado(o, "LISTA")}
            className="d-inline-flex align-items-center gap-2">
        ...
        {est === "LISTA" ? (
          <Button size="sm" variant="dark" disabled={disabled} onClick={() => cambiarEstado(o, "ENTREGADA")}
            className="d-inline-flex align-items-center gap-2">
            {disabled ? <Spinner size="sm" animation="border" /> : <FaCheckCircle />}
            Entregar
          </Button>
        ...
        {["EN_PREPARACION", "LISTA"].includes(est) ? (
          <Button size="sm" variant="outline-secondary" disabled={disabled} onClick={() => cambiarEstado(o, "NUEVA")}
            className="d-inline-flex align-items-center gap-2">
            <FaUndo />
            Regresar
          </Button>
        ...
        {["CANCELADA", "ENTREGADA"].includes(est) ? null : (
          <Button size="sm" variant="outline-danger" disabled={disabled} onClick={() => cambiarEstado(o, "CANCELADA")}
            className="d-inline-flex align-items-center gap-2">
            <FaBan />
            Cancelar
          </Button>
  ...
  return (
    <Container fluid className="py-3">
          ...
          <div className="d-flex align-items-center gap-2">
            ...
            <div>
              <div className="fw-bold" style={{ fontSize: 18 }}>
                Órdenes (Monitor) <Badge bg="success" className="ms-2">LIVE</Badge>
              </div>
      ...
      <Row className="g-3 mb-3">
        {[
          { label: "Total", val: kpis.total, bg: "light", text: "dark" },
          { label: "Nuevas", val: kpis.nueva, bg: "secondary" },
          { label: "Preparación", val: kpis.prep, bg: "warning", text: "dark" },
          { label: "Listas", val: kpis.lista, bg: "success" },
          { label: "Entregadas", val: kpis.entregada, bg: "dark" },
          { label: "Canceladas", val: kpis.cancelada, bg: "danger" },
        ].map((k) => (
          <Col key={k.label} xs={6} md={4} lg={2}>
            <Card className="shadow-sm border-0 rounded-4">
              <Card.Body className="py-3">
                <div className="text-muted" style={{ fontSize: 12 }}>{k.label}</div>
                <div className="fw-bold" style={{ fontSize: 22 }}>{k.val}</div>
              </Card.Body>
            </Card>
          </Col>
        ))}
                ...
                <Card key={o.id} className="shadow-sm border-0 rounded-4">
                  <Card.Body>
                    <div className="d-flex justify-content-between align-items-start gap-2">
                      <div>
                        <div className="fw-bold" style={{ fontSize: 16 }}>{o.codigo}</div>
                        <div className="text-muted" style={{ fontSize: 12 }}>{fmtDateTime(o.createdAt)}</div>
                        <div className="mt-1 d-flex gap-2 flex-wrap">
                          <Badge bg="dark">{o.tipo}</Badge>
                          {o.tipo === "MESA" ? <Badge bg="info" text="dark">Mesa {o.mesa ?? "—"}</Badge> : null}
                      ...
                      <div className="text-end">
                        ...
                        <div className="fw-bold">{o.total == null ? "—" : money(o.total)}</div>
                      </div>
                    </div>
                    ...
                    <div className="d-flex justify-content-between text-muted" style={{ fontSize: 12 }}>
                      <span>Items: <b>{safeArray(o.items).length || o.items_count || 0}</b></span>
                    </div>
              ...
              <Table responsive hover className="mb-0 align-middle">
                <thead style={{ position: "sticky", top: 0, zIndex: 1, background: "white" }}>
                  <tr>
                    ...
                    <th style={{ minWidth: 120 }} className="text-end">Total</th>
                    <th style={{ minWidth: 280 }} className="text-end">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {filtradas.length === 0 ? (
                    ...
                    filtradas.map((o) => (
                      <tr key={o.id}>
                        ...
                        <td className="text-muted" style={{ fontSize: 12 }}>{fmtDateTime(o.createdAt)}</td>
                        <td><Badge bg="dark">{o.tipo}</Badge></td>
                        <td>{o.tipo === "MESA" ? (o.mesa ?? "—") : "—"}</td>
                        <td>{badgeEstado(o.estado)}</td>
                        <td className="fw-semibold">{safeArray(o.items).length || o.items_count || 0}</td>
      ...
      <Modal show={showDetalle} onHide={() => setShowDetalle(false)} size="lg" centered>
        <Modal.Header closeButton>
          <Modal.Title className="fw-bold">
            Detalle{" "}
            {ordenSel?.codigo ? <Badge bg="dark" className="ms-2">{ordenSel.codigo}</Badge> : null}
          </Modal.Title>
        </Modal.Header>
                ...
                <Col md={6}>
                  <Card className="rounded-4 border">
                    <Card.Body className="py-3">
                      ...
                      <div className="text-muted small mt-1">Fecha: {fmtDateTime(ordenSel.createdAt)}</div>
                    </Card.Body>
                  </Card>
                </Col>
